...
Status: implemented baseline; selective-adoption evidence refreshed 2026-09-17. Earlier dated compatibility observations retain their original scope.
...
The source `config.toml` failed the schema checked during the 2026-08-31 review because `windows_wsl_setup_acknowledged` was not a recognized top-level property. The tagged Codex `0.153.4` schema and live schema rechecked on 2026-09-07 are byte-identical to the prior `0.153.2` snapshot and still omit that property, although the current configuration reference lists it as a Windows-only boolean. The reference likewise describes `agents.max_threads` as a legacy alias, while the `0.153.4` schema exposes only `agents.max_concurrent_threads_per_session`; public examples therefore use the canonical key. Treat these differences as documentation/schema mismatches and report schema acceptance separately from runtime support. An opaque `[desktop]` table may pass schema validation because it permits additional properties, but that does not prove arbitrary keys have runtime effect. These conclusions were checked against the current [Codex config reference](https://learn.chatgpt.com/docs/config-file/config-reference), [configuration schema](https://developers.openai.com/codex/config-schema.json), and [hooks documentation](https://learn.chatgpt.com/docs/hooks).
...
## Addy Osmani Agent Skills Selective Adoption

The 2026-09-17 source review pinned [`addyosmani/agent-skills` at `be4e44a`](https://github.com/addyosmani/agent-skills/tree/be4e44a9fbc5e8df0beaefadbb28bd22ee61cc39) and inspected its [MIT license](https://github.com/addyosmani/agent-skills/blob/be4e44a9fbc5e8df0beaefadbb28bd22ee61cc39/LICENSE). Its 25-Skill coding lifecycle is useful comparative material, not the target taxonomy for this document-delivery repository. Compatible mechanisms were independently expressed; no upstream Skill body or executable code was copied.

| Candidate and source | Decision | Local result or re-entry condition |
|---|---|---|
| [Pressure cases](https://github.com/addyosmani/agent-skills/blob/be4e44a9fbc5e8df0beaefadbb28bd22ee61cc39/evals/README.md) and resistance to rationalizing skipped checks | Adopt as evaluation material | Add normal/pressure pairs for static-evidence overclaims and prior-reviewer anchoring. Do not add anti-rationalization tables to every Skill without a demonstrated gap. |
| [Independent review discipline](https://github.com/addyosmani/agent-skills/blob/be4e44a9fbc5e8df0beaefadbb28bd22ee61cc39/skills/code-review-and-quality/SKILL.md) and [constraint discipline](https://github.com/addyosmani/agent-skills/blob/be4e44a9fbc5e8df0beaefadbb28bd22ee61cc39/skills/constraint-driven-development/SKILL.md) | Adapt to document evidence and authority | Recheck each inherited finding; test unapproved criterion relaxation alongside a valid limited-pilot approval and exception. Preserve historical failures without inventing an absolute prohibition on requirement changes. |
| [Different adoption paths for new and established projects](https://github.com/addyosmani/agent-skills/blob/be4e44a9fbc5e8df0beaefadbb28bd22ee61cc39/README.md) | Adopt with narrower scope | Start with the actual deliverable on either path; preserve established authority and approvals. Usage can be gradual even though the existing installer manages all nine Skills. |
| Whole coding lifecycle, meta-router, default hooks, mandatory reviewer/cross-model steps, and a numeric 95% confidence target | Do not adopt | No distinct document-delivery gap justifies the extra scope, loading cost or unsupported certainty. Existing specialist ownership and evidence-based completion remain authoritative. |
| Upstream routing scorer and execution harness | Do not import | The inspected routing tokenizer drops Chinese text and its description regex reads folded YAML as `>-`; the behavior harness supplies Skill text explicitly to Claude. Its scores do not establish this repository's multilingual routing or net benefit. Reopen only for a reproduced routing defect and a task-appropriate comparison. |
| Plugin packaging, capability map, rejected-change ledger | Defer | Reopen packaging for an observed distribution need; a map for repeated ownership/discovery failures; a ledger for repeatedly reconsidered consequential alternatives that existing decision records cannot cover. Do not add new competing authorities by default. |

The [bounded Codex pilot](agent-skills-pilot.md) ran six Chinese cases against existing `rd-research` and `rd-review` text, with 24/24 assertions passing under task-agent semantic grading. No attributable instruction gap was found, so Skill bodies, descriptions, routing and permissions remain unchanged. The full responses and grading evidence are durable repository artifacts. This is one observation per case, not an independent benchmark, native-discovery test, cross-client result, or causal improvement claim; there was no no-Skill control. Other reviewers' reported run counts were not rerun or adopted as this project's execution evidence.
...
- At least three output-quality evals per Skill; after integrating the released v1.7.4 design eval, the 2026-09-17 canonical English and Chinese roots each contain 42 cases: 36 from v1.7.4 plus six fixture-backed adoption cases. The earlier pilot baseline contained 41 including those six.
...
- Repository validation uses real YAML, TOML, JSON, and JSON-Schema parsers, then verifies Skill structure and references, LF endings, mirror equality, value-first and evidence contracts, Codex example schema/runtime-baseline semantics, Cursor's exactly-one always-on rule policy, cross-platform reasoning governance, Context7 version consistency, platform prompt prefixes, Claude Bash/PowerShell permission and attribution settings, Cursor MCP credential transport, Unix installation prerequisites, maintainer/context/prompt contracts, current GSD attribution, and the `.tmp/local/` boundary
- A separate twenty-case negative regression runner retains the sixteen existing Skill, evidence, global-rule, context, prompt, temporary-data, and platform-configuration cases and adds Codex configuration-contract, Cursor loading-policy, cross-platform reasoning-governance, and current-versus-archived GSD attribution regressions

The JSON eval files define routing and output expectations but do not themselves execute models. The 2026-09-17 pilot supplies bounded execution evidence for six Chinese cases only. Runtime evaluation should progress from an environment and authentication smoke test to changed-surface cases and related regression; run the full Codex/Claude/Cursor with-Skill and comparison matrix only for shared routing, public contracts, release decisions, observed cross-surface risk, or an explicit request. Record selection, assertions, tokens, and elapsed time at the evidence level actually reached. The next description change should therefore come from observed false triggers, missed triggers, premature step completion, broken handoffs, duplicate authority records, or low-quality real outputs. Adding another top-level Skill without an independently triggerable deliverable or workflow would be speculative.
